...
async def antiraid(interaction: discord.Interaction, enabled: bool = True):
    ...
    guild_data = bot_data.setdefault(str(interaction.guild.id), {})
    guild_data['raid_protection_enabled'] = enabled
    save_data(bot_data)
...
async def addword(interaction: discord.Interaction, word: str):
    ...
    guild_data = bot_data.setdefault(str(interaction.guild.id), {})
    banned_words = guild_data.setdefault('banned_words', [])
...
async def bannedwords(interaction: discord.Interaction):
    ...
    guild_data = bot_data.get(str(interaction.guild.id), {})
    banned_words = guild_data.get('banned_words', [])
...
async def on_message(message):
    ...
    guild_data = bot_data.setdefault(str(message.guild.id), {})
    
    maintenance_mode = guild_data.get('maintenance_mode', False)
    maintenance_reason = guild_data.get('maintenance_reason', "")
    automod_enabled = guild_data.get('automod_enabled', False)
    banned_words = guild_data.get('banned_words', [])
    anti_spam = guild_data.setdefault('anti_spam', {})
    ...
    if automod_enabled and not message.author.guild_permissions.administrator:
        ...
        anti_spam.setdefault(user_id, [])
        anti_spam[user_id] = [
            msg_time for msg_time in anti_spam[user_id] 
...
async def on_member_join(member):
    ...
    guild_data = bot_data.get(str(member.guild.id), {})
    raid_protection_enabled = guild_data.get('raid_protection_enabled', False)
    log_channel_id = guild_data.get('log_channel_id')
